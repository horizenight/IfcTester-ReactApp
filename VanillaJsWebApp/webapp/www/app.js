...
class IDSSpecs extends HTMLElement {
    load(idsElement) {
        ...
        this.idsElement.addEventListener('ids-spec', function() { self.render(); });
        this.render();
    }
...
class IDSSpec extends HTMLElement {
   ...
   dragEnd(e){
        e.preventDefault();
        const src = document.querySelectorAll('[dragging]')[0];
        const containers = document.querySelectorAll('ids-specs')[0];
        
        const afterElement = getDragAfterElement(containers, e.clientY);

        if(afterElement == null){
            containers.idsElement.appendChild(src.idsElement);
            // this.idsElement.parentElement.insertBefore(this.idsElement, nextNextSibling);
        }else{
            containers.idsElement.insertBefore(src.idsElement, afterElement.idsElement)
        }


            
        function getDragAfterElement(parent, y, draggingElement) {
            const draggableElements = [...parent.querySelectorAll('ids-spec[draggable]:not([dragging])')];
        
            return draggableElements.reduce((closest, child) => {
                if (child === draggingElement) {
                    return closest; // Skip the currently dragged element
                }
        ...
        this.idsElement.parentElement.dispatchEvent(new Event('ids-spec'));
   }
